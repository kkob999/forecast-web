...
curl_setopt_array(
    $curl,
    array(
        CURLOPT_URL => 'https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&timezone=GMT&null=null&daily=windspeed_10m_max%2Cweathercode%2Cprecipitation_probability_mean%2Cet0_fao_evapotranspiration&forecast_days=14&daily=temperature_2m_max%2Ctemperature_2m_min%2Cuv_index_max%2Csunrise%2Csunset',
...
for ($i = 0; $i < sizeof($time_arr); $i++) {
    ...
    $humid = $data['daily']['et0_fao_evapotranspiration'][$i];
    ...
    $sql = "INSERT INTO weekly_weather(id, latitude, longitude, high_temp,low_temp, humid,  wind_speed,  rain, icon,  uv,  sunrise,sunset ,time)
VALUES('$i','$latitude', '$longitude', '$high_temp', '$low_temp','$humid', '$wind_speed', '$rain', '$icon', '$uv', '$sunrise', '$sunset', '$time')";
    ...
    <div class="container" style="height: 100%; overflow: overflow-x: unset;">
        ...
        <div style="width: 20%; position: fixed; height: 100%;">
            <div class="nav">
                ...
                VALUES('$i','$latitude', '$longitude', '$high_temp', '$low_temp','$humid', '$wind_speed', '$rain', '$icon', '$uv', '$sunrise', '$sunset', '$time')";
                                ...
                                CloseCon($conn);
                                ?>
                                document.getElementById("demo").innerHTML = "Delete Data";
                            }
        ...
        <div style="width: 80%; margin-left: 20%;">
            ...
            <div class="search">
                ...
                <button><img style="width: 30px; height: 30px;" src="./img/search.png"></button>
                <script>
                    const searchBox = document.querySelector(".search input")
                    const searchBtn = document.querySelector(".search button")
                    async function handleClick(city) {
                        let url = 'http://api.openweathermap.org/geo/1.0/direct?q=' + city + '&appid=207b5ebcf8768062b41364ba2f183b0d';
                        const res = await fetch(url)
                        const json = await res.json()
                        // console.log(json[0].name + " " + json[0].lat)
                        document.getElementById("city").innerHTML = " in " + json[0].name;
                        return [json[0].name, json[0].lat, json[0].lon]

                    }
                    searchBtn.addEventListener("click", async () => {

                        const data = await handleClick(searchBox.value);
                        console.log(data[1].toFixed(2) + " and " + data[2].toFixed(2))
                        //assign varaible
                        document.cookie = "lat = " + data[1].toFixed(2);
                        document.cookie = "lon = " + data[2].toFixed(2);
                        ...
                        $url_week = 'https://api.open-meteo.com/v1/forecast?latitude=' . $lat . '&longitude=' . $lon . '&timezone=GMT&null=null&daily=windspeed_10m_max%2Cweathercode%2Cprecipitation_probability_mean%2Cet0_fao_evapotranspiration&forecast_days=14&daily=temperature_2m_max%2Ctemperature_2m_min%2Cuv_index_max%2Csunrise%2Csunset';
            ...
            <div class="card">
                <div style="margin: 0 40px 0 40px;">
                    <h1>14 Days Weather</h1>
                    <h1 id="city">in london</h1>
                    <?php
                    while ($rows = $result->fetch_assoc()) {
                        ...
                        <div class="flex-row">
                            ...
                            <div style="width: 30%; align-self: center;" class="al flex-row">
                                <div style="margin-right: 5%; width: 30%;;" class="flex-row">
